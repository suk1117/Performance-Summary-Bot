...
def _history_path(uid: int, pname: str) -> str:
    return os.path.join(_user_dir(uid), f"history_{pname}.json")

def _cashflow_path(uid: int, pname: str) -> str:
    return os.path.join(_user_dir(uid), f"cashflow_{pname}.json")
...
def _build_portfolio_tabs(all_portfolios: dict, current_pname: str, uid: int = 0) -> str:
    """포트폴리오 탭 HTML 생성"""
    ...
    for pname, p in all_portfolios.items():
        ...
        if pname == current_pname:
            ...
            tabs += f'<a href="/u/{uid}/p/{pname}" class="tab">{name}</a>'
    tabs += (
...
def build_user_html(
    ...
    portfolio_tabs = _build_portfolio_tabs(all_portfolios or {}, pname, uid)
...
window.toggleIndex = async function(name) {{
  ...
  if (!_idxCache[name] && _histStart) {{
    try {{
      const r = await fetch("/api/index_returns?start=" + _histStart);
...
def index():
    ...
    if active_pname and active_pname in portfolios:
        return redirect(f"/u/{TELEGRAM_CHAT_ID}/p/{active_pname}")
    if portfolios:
        return redirect(f"/u/{TELEGRAM_CHAT_ID}/p/{next(iter(portfolios))}")
...
def portfolio_page(uid: int, pname: str):
    ...
    p  = portfolios[pname]
    df = p["df"]
    # 오늘 스냅샷이 없으면 현재가 조회 후 자동 저장 (차트 표시용)
    if df is not None and len(df) > 0:
        ...
        if today not in hist_check:
            try:
                df = build_dashboard_for(uid, pname)
            except Exception:
                pass
...
def api_delete_portfolio(uid: int, pname: str):
    ...
    return jsonify({"ok": True, "redirect": f"/u/{uid}/p/{new_active}"})
...
@app_flask.route("/api/index_returns")
def api_index_returns():
...
def _summary_text(uid: int, pname: str) -> str:
    ...
    url   = f"{public_url}/u/{uid}/p/{pname}"
...
async def handle_excel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        df = await asyncio.get_event_loop().run_in_executor(None, build_dashboard_for, uid, active_pname)
        ...
        log.info(f"✅ 처리 완료 → uid={uid} {active_pname}")
...
async def cmd_run(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    state = _get_user_state(uid)
    active_pname = state["active_pname"]
    url = f"{public_url}/u/{uid}/p/{active_pname}" if active_pname else public_url
    await update.message.reply_text(
        f"📊 *대시보드 URL*\n\n🔗 {url}",
        parse_mode=ParseMode.MARKDOWN,
...
async def cmd_refresh(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await asyncio.get_event_loop().run_in_executor(None, build_dashboard_for, uid, active_pname)
        ts  = portfolios[active_pname]["last_update"].strftime("%m/%d %H:%M")
        url = f"{public_url}/u/{uid}/p/{active_pname}"
        await msg.edit_text(
            f"✅ *업데이트 완료*\n\n🕐 `{ts} KST`\n🔗 {url}",
            parse_mode=ParseMode.MARKDOWN,
        )
    ...
    for uid, state in list(all_users.items()):
        ...
        for pname in list(portfolios.keys()):
            ...
            if p.get("df") is None or len(p.get("df", [])) == 0:
                continue
            try:
                await asyncio.get_event_loop().run_in_executor(None, build_dashboard_for, uid, pname)
